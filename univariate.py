...
class MetricsPlotter:
    def __init__(self, mse, val_mse, loss, val_loss):
        ...
        self.epochs = len(self.mse)  # Get the number of epochs

    def plot(self, title=None):
        # plot the mse values on the first subplot
        self.ax1.plot(range(self.epochs), self.mse, label='Training MSE')
        self.ax1.plot(range(self.epochs), self.val_mse, label='Validation MSE')
        self.ax1.set_ylabel('MSE')
        self.ax1.legend()

        # plot the loss values on the second subplot
        self.ax2.plot(range(self.epochs), self.loss, label='Training Loss')
        self.ax2.plot(range(self.epochs), self.val_loss, label='Validation Loss')
        self.ax2.set_ylabel('Loss')
        self.ax2.legend()

        # set the x-axis label and tick marks
        self.ax2.set_xlabel('Epoch')
        self.ax2.set_xticks(range(0, self.epochs, self.epochs//10))  # Adjust the tick interval as desired
...
def load_data(col=None, path=None, verbose=False):
    if path is None:
        return None
...
# data with time as feature
imput_col_time = ['Heating']

multivar_df_time = load_data(imput_col_time,path='.\Kavgic Years.csv', verbose = True )
multivar10_19 = load_data(imput_col_time,path='.\k2010-2019.csv',verbose = True )
...
List_S_fea = [multivar10_19,multivar11_19,multivar12_19,multivar13_19,multivar14_19, multivar15_19,
              multivar16_19 , multivar_df_time,]
...
for j in s:
    def split_data(series):
        """Splits input series into train, val and test.
        """
        train_data = series[:78840] # 2011 start from 70080 done saved 11 , 61320 , 52560 , 43800
        test_data = series[78840:]
        return train_data, test_data
...
dataset = hstack(( in_seqtHeating.reshape(-1, 1), out_seqt.reshape(-1, 1)))
out_seqt_test = in_seqt_testHeating
dataset_test = hstack(( in_seqt_testHeating.reshape(-1, 1), out_seqt_test.reshape(-1, 1)))
...
# save the model
model_cnn_lstm.save('CNNLSTM0.h5')
...
# Save the model
hybrid_rnn_lstm_model.save('RNNLSTM0.h5')
...
end_time = time.time()
times.append(end_time - start_time)
metrics_df.to_csv('metric11', index=False)

import csv
# Specify the CSV file path
csv_file = 'Times10.csv'
...
for j in s:
    def split_data(series):
        """Splits input series into train, val and test.
        """
        train_data = series[:70080] # 2011 start from 78840 2010 , 70080 done saved 11 , 61320 , 52560 , 43800
        test_data = series[70080:]
        return train_data, test_data
    vars()[f"in_seqt{j}"] = train_multi[j].values
    vars()[f"in_seqt{j}"].reshape((len(train_multi[j]), 1))
    vars()[f"in_seqt_test{j}"] = test_multi[j].values
...
# PostProcessing
# create an instance of the MetricsPlotter class
# = MetricsPlotter(mse, val_mse, loss, val_loss)
#metrics_plotter.plot(title='RNN LSTM')
yhat1 = hybrid_rnn_lstm_model.predict(X1, verbose=0)
num_cols = n_steps_out
# Generate column names
